...
export default function ThemeEditorVariables({ vars_light, vars_dark, onChange }: ThemeEditorVariablesProps) {
    return (
        <Card>
                        ...
                        <div className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2 md:grid-cols-3">
                            {COLOR_KEYS.map(key => (
                                <ColorPicker
                                    ...
                                    onChange={(val) => onChange('light', key, val)}
                                />
                            ))}
                        </div>
                        <div className="pt-4 border-t">
                            <div className="w-full sm:w-1/3">
                                <Label htmlFor="radius-light" className="text-[10px] font-bold tracking-wider text-muted-foreground uppercase">Radius</Label>
                                <Input
                                    id="radius-light"
                                    value={vars_light['radius'] || '0.5rem'}
                                    onChange={(e) => onChange('light', 'radius', e.target.value)}
                        ...
                        <div className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2 md:grid-cols-3">
                            {COLOR_KEYS.map(key => (
                                <ColorPicker
                                    ...
                                    onChange={(val) => onChange('dark', key, val)}
                                />
                            ))}
                        </div>
                    </TabsContent>
